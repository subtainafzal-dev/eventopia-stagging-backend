...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'event_status_enum') THEN
    CREATE TYPE event_status_enum AS ENUM ('draft','pending_approval','active','published','completed','cancellation_requested','cancelled','unpublished');
  END IF;
...
CREATE TABLE IF NOT EXISTS refund_cases (
  ...
  escrow_ring_fenced BOOLEAN NOT NULL DEFAULT FALSE,
...
ALTER TABLE refund_cases
  DROP CONSTRAINT IF EXISTS refund_cases_reason_code_check;

CREATE INDEX IF NOT EXISTS idx_refund_cases_buyer ON refund_cases(buyer_id, submitted_at DESC);
CREATE INDEX IF NOT EXISTS idx_refund_cases_event ON refund_cases(event_id, submitted_at DESC);
CREATE INDEX IF NOT EXISTS idx_refund_cases_status ON refund_cases(status, submitted_at DESC);
CREATE INDEX IF NOT EXISTS idx_refund_cases_order_item ON refund_cases(order_item_id);

CREATE UNIQUE INDEX IF NOT EXISTS uq_refund_cases_open_order_item
ON refund_cases(order_item_id)
...
/* ================================
   CONSOLIDATED MIGRATION PATCHES
   (merged from migrations/run_migration.sql + 003/004/005)
   Keep idempotent so init.sql remains re-runnable.
   ================================ */

-- Users / territories compatibility columns
ALTER TABLE users ADD COLUMN IF NOT EXISTS territory_id BIGINT REFERENCES territories(id) ON DELETE SET NULL;
ALTER TABLE users ADD COLUMN IF NOT EXISTS guru_id BIGINT REFERENCES users(id) ON DELETE SET NULL;
...
-- Event publish/edit compatibility fields
...
ALTER TABLE events ADD COLUMN IF NOT EXISTS views_count BIGINT NOT NULL DEFAULT 0;

-- Escrow compatibility for both legacy and new query shapes
ALTER TABLE escrow_accounts ADD COLUMN IF NOT EXISTS balance BIGINT NOT NULL DEFAULT 0;
ALTER TABLE escrow_accounts ADD COLUMN IF NOT EXISTS pending_liabilities BIGINT NOT NULL DEFAULT 0;
ALTER TABLE escrow_accounts ADD COLUMN IF NOT EXISTS coverage_ratio NUMERIC(10,4);
ALTER TABLE escrow_accounts ADD COLUMN IF NOT EXISTS interest_earned BIGINT DEFAULT 0;

-- Event-level escrow liability table
...
-- Event status enum values from incremental migrations
